...
class MagangController extends BaseController
{
    ...
    public function pesertaMagang()
    {
        ...
        */
        $builder = $this->magangModel
            ->select('
                ...
                users.fullname,
                users.nisn_nim,
                users.tingkat_pendidikan,

                -- safety terakhir (TIDAK pakai MAX lagi)
                js.nilai as nilai_maksimal,
                ...
                penilaian.approve_kaunit,
                penilaian.tgl_disetujui,
                penilaian.approve_by,
                penilaian.catatan,
                penilaian.catatan_approval,
            ...
            ->where('magang.status_akhir', 'magang'); // default peserta aktif
        ...
        */
        if ($bulanMasuk) {
            ...
            $builder->where('magang.tanggal_masuk <', date('Y-m-01', strtotime($bulanMasuk . ' +1 month')));
        }

        if ($bulanKeluar) {
            $builder->where('magang.tanggal_selesai >=', $bulanKeluar . '-01');
            $builder->where('magang.tanggal_selesai <', date('Y-m-01', strtotime($bulanKeluar . ' +1 month')));
        }
        ...
        */
        $chartData = [
            'SMK' => ['aktif' => 0, 'akan_masuk' => 0, 'belum_lulus' => 0],
            'Perguruan Tinggi' => ['aktif' => 0, 'akan_masuk' => 0, 'belum_lulus' => 0]
        ];

        foreach ($data as $row) {

            $tingkat = in_array($row['tingkat_pendidikan'], ['D3','D4/S1','S2'])
                ? 'Perguruan Tinggi'
                : ($row['tingkat_pendidikan'] === 'SMK' ? 'SMK' : null);
    ...
    public function pesertaMagangold2()
    {
        $unitId  = $this->request->getGet('unit_kerja');
        $bulanMasuk  = $this->request->getGet('tanggal_masuk'); 
        $bulanKeluar = $this->request->getGet('tanggal_keluar');
    ...
    public function alumniMagang()
    {
        ...
        */
        ...
        /*
        |--------------------------------------------------------------------------
        | Subquery Safety Terakhir (GANTI MAX + GROUP BY)
        |--------------------------------------------------------------------------
        */
        $subSafety = "
            (
        ...
        */
        $builder = $this->magangModel
            ...
            ->join($subPenilaian, 'p.magang_id = magang.magang_id', 'left')
            ->join($subSafety, 'js.relasi_id = magang.magang_id', 'left')
            ->join($subRfid, 'ra.relasi_id = magang.magang_id', 'left')
            ...
            ->where('magang.status_akhir', 'lulus'); // alumni only
    ...
    public function alumniMagangold2()
    {
        $bulan = $this->request->getGet('bulan');
        $tahun = $this->request->getGet('tahun');
