...
<h1 class="h3 mb-4 text-gray-800">Validasi Konfirmasi Penelitian</h1>
<div class="card shadow mb-4">
    <div class="card-body">
        <form action="<?= base_url('admin/penelitian/validasi-konfirmasi/bulk') ?>" method="post" id="bulkForm">
            <?= csrf_field() ?>
            <div class="mb-3">
                                ...
                                <tr>
                                    <td>
                                        <?php if ($item['status_konfirmasi'] === 'Y'): ?>
                                            <input type="checkbox" name="ids[]" value="<?= esc($item['penelitian_id']) ?>" class="checkbox-pendaftar">
                                        <?php endif; ?>
                                    </td>
                                    <td><?= esc($item['fullname']) ?></td>
                                    <td><?= esc($item['nisn_nim']) ?></td>
                                    <td><?= $item['status_konfirmasi'] === 'Y' ? 'Terkonfirmasi' : 'Tidak Konfirmasi' ?></td>
                                    <td><?= esc(format_tanggal_indonesia($item['tanggal_konfirmasi'])) ?></td>
                                </tr>
                            <?php endforeach; ?>
                        <?php endif; ?>
                    </tbody>
